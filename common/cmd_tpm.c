		...
		int argc, char * const argv[])
{
	...
	if (!strcmp("TPM_ST_CLEAR", argv[1])) {
		mode = TPM_ST_CLEAR;
	} else if (!strcmp("TPM_ST_STATE", argv[1])) {
		mode = TPM_ST_STATE;
	} else if (!strcmp("TPM_ST_DEACTIVATED", argv[1])) {
...
static int do_tpmutil_nv_define(cmd_tbl_t *cmdtp, int flag,
		int argc, char * const argv[])
{
	uint32_t index, perm, size;
...
static int do_tpmutil_nv_read(cmd_tbl_t *cmdtp, int flag,
		int argc, char * const argv[])
{
...
static int do_tpmutil_nv_write(cmd_tbl_t *cmdtp, int flag,
		int argc, char * const argv[])
{
	uint32_t index, count, err;
	void *data;
...
static cmd_tbl_t tpmutil_commands[] = {
	U_BOOT_CMD_MKENT(raw_transfer, 0, 1,
			do_tpmutil_raw_transfer, "", ""),
	U_BOOT_CMD_MKENT(nv_define, 0, 1,
			do_tpmutil_nv_define, "", ""),
...
static int do_tpmutil(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	cmd_tbl_t *tpmutil_cmd;

	if (argc < 2)
		return CMD_RET_USAGE;
	tpmutil_cmd = find_cmd_tbl(argv[1],
			tpmutil_commands, ARRAY_SIZE(tpmutil_commands));
	if (!tpmutil_cmd)
		return CMD_RET_USAGE;

	return tpmutil_cmd->cmd(cmdtp, flag, argc - 1, argv + 1);
}
...
U_BOOT_CMD(tpmutil, CONFIG_SYS_MAXARGS, 1, do_tpmutil,
"Miscellaneous TPM helper functions",
"func args...\n"
"    - Run helper function <func> with arguments <args...>.\n"
"List of helper functions:"
"  raw_transfer byte_string\n"
"    - Send a byte string <byte_string> to TPM and print the response.\n"
"Helper functions for accessing TPM non-volatile storage:\n"
